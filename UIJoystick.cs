...
namespace TanksMP
{
    ...
    /// </summary>
    public class UIJoystick : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler,IPointerUpHandler,IPointerDownHandler,IPointerEnterHandler
    {
        /// <summary>
        ...
        public event Action<Vector2> onEnter;

        public event Action onEnterDown;

        public event Action onEnterUp;
        ...
        public GameObject UiProjectile;
        public GameObject UiLandMine;
        ...
        private bool m_checkDrawLineProjectile = false;
        ...
        void Update()
        {
            ...
            if(player && UiProjectile && UiBullet && UiLandMine)
            {
                if (player.projectile == false && player.landmine == false)
                    UIBullet();
                else if (player.projectile == true && player.landmine == false)
                    UIProjectile();
                else if (player.landmine == true && player.projectile == false)
                    UILandmine();
                else
                {
                    UIBullet();
        ...
        public void UIProjectile()
        {
            UiProjectile.SetActive(true);
            UiBullet.SetActive(false);
            UiLandMine.SetActive(false);
        }
        public void UILandmine()
        {
            UiProjectile.SetActive(false);
            UiBullet.SetActive(false);
            UiLandMine.SetActive(true);
        }
        public void UIBullet()
        {
            UiProjectile.SetActive(false);
        ...
        public void OnPointerUp(PointerEventData eventData)
        {
            ...
            m_checkDrawLineProjectile = false;
        }
        ...
        {
            m_checkDrawLineProjectile = true;
        }
